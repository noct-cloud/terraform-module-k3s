...
resource null_resource root_server_install {

  depends_on = [var.depends_on_, null_resource.k8s_ca_certificates_install]
  triggers = {
    on_immutable_changes = local.servers_metadata[local.root_server_name].immutable_fields_hash
  ...
  provisioner "remote-exec" {
    inline = [
      "INSTALL_K3S_VERSION=${local.k3s_version} sh /tmp/k3s-installer server ${local.servers_metadata[local.root_server_name].flags}",
      "until kubectl get node ${local.root_server_name}; do sleep 1; done"
    ]
  }
}
...
resource null_resource servers_install {
  ...
  depends_on = [var.depends_on_, null_resource.root_server_install]
  triggers = {
